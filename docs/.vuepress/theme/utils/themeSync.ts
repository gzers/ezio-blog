...
import { darkSymbol } from "@/utils/customThemeDefault"

const theme = useThemeStore(pinia)
const classSymbol = darkSymbol

export function setThemeConfig() {
    const defaultThemeSelected = localStorage.getItem('faq-vuepress-theme-selected')
    if (defaultThemeSelected) {
        theme.changeTheme(JSON.parse(defaultThemeSelected))
    } else {
        theme.changeThemeDefault()
    }
}
